...
/// JWT ile giriş yapmış panel kullanıcıları yalnızca tenant-kapsamlı yollarda muaftır;
/// sistem-only yollar (reminders/*) JWT ile ATLANAMAZ.
/// </summary>
public class WebhookAuthMiddleware
{
    ...
    public async Task InvokeAsync(HttpContext context, WebhookAuthValidator validator)
    {
        ...
        // Panel JWT kullanıcıları yalnızca tenant-kapsamlı webhook yollarında muaftır.
        // Sistem-only yollar (çok kiracılı cron) JWT ile atlanamaz — sistem token'ı zorunlu.
